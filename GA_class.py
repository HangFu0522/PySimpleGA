...
class Population:
    ...
    def the_best(self):
        self.bestfunvalue = self.fun_value[0]
        self.bestone = []

        for i in range(self.number):
            if not self.bestfunvalue > self.fun_value[i]:
                self.bestone = self.individual_list[0].gene[:]
                self.bestfunvalue = self.fun_value[i]

    def get_best_one(self):
        return self.bestone

    def get_best_value(self):
        return self.bestfunvalue
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        pop.the_best()
        best_one.append(pop.get_best_one())
        best_value.append(pop.get_best_value())

        pop.cross()
        pop.mutate()

    best_index = best_value.index(max(best_value))
    best = best_one[best_index]
